...
+(id) scene
{
    ...
    Scene04 *layer = [[Scene04 alloc] initWithHUD:hud];
...
-(void)onEnterTransitionDidFinish{
    ...
    id wait2 = [CCDelayTime actionWithDuration: 3.5];
    ...
    [_timesSquare runAction:[CCSequence actions:wait,pop, [CCCallFuncN actionWithTarget:self selector:@selector(playPopSound)], nil]];
    
    //Central Park
    
    [_centralPark runAction:[CCSequence actions:wait2, pop, [CCCallFuncN actionWithTarget:self selector:@selector(playPopSound)], nil]];
...
- (void)onExitTransitionDidStart{
    ...
    [_centralPark stopAllActions];
    [_timesSquare stopAllActions];
...
- (void)handleViewsSwipeLeft:(UISwipeGestureRecognizer *)gesture {
    if (readText == NO ) {
        ...
        [[CCDirector sharedDirector] replaceScene:[CCTransitionPageTurn transitionWithDuration:2.0 scene:[Scene05 scene] ]];
    }
...
- (void)handleViewsSwipeRight:(UISwipeGestureRecognizer *)gesture {
    ...
    if (readText == NO ) {
        ...
        [[[CCDirector sharedDirector] view] removeGestureRecognizer:Swiperight];
        [[[CCDirector sharedDirector] view] removeGestureRecognizer:Swipeleft];
        [[CCDirector sharedDirector] replaceScene:[CCTransitionMoveInL transitionWithDuration:.5 scene:[Scene03 scene] ]];
    }
...
- (void)setUpScene {
    ...
    //Times Square
    _timesSquare  = [CCSprite spriteWithSpriteFrameName:@"timesSquare.png"];
    _timesSquare.position = CGPointMake(winSize.width*.25,winSize.height*.45);
    [_timesSquare setScale:.6];
    [self addChild:_timesSquare];
    
    
    //Central Park
    _centralPark  = [CCSprite spriteWithSpriteFrameName:@"centralPark.png"];
    _centralPark.position = CGPointMake(winSize.width*.75,winSize.height*.45);
    [ _centralPark setScale:.6];
...
- (void)readText
{
    ...
    if (readText == NO)
    {
        ...
        CCAction *playSound = [CCCallBlock actionWithBlock:^{
            ...
            [[SimpleAudioEngine sharedEngine] playEffect:@"pg04.mp3"];
...
- (BOOL)ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    }    else if (CGRectContainsPoint(_timesSquare.boundingBox, touchLocation)) {
        
      
        
        CCAction *sequence = [CCSequence actions:[CCRotateBy actionWithDuration:.25 angle:20], [CCRotateBy actionWithDuration:.25 angle:-40],[CCRotateBy actionWithDuration:.25 angle:20], nil];
        [_timesSquare runAction:sequence];
    ...
    else if (CGRectContainsPoint(_centralPark.boundingBox, touchLocation)) {
        
      
        CCAction *sequence = [CCSequence actions:[CCRotateBy actionWithDuration:.25 angle:20], [CCRotateBy actionWithDuration:.25 angle:-40],[CCRotateBy actionWithDuration:.25 angle:20], nil];
        [_centralPark runAction:sequence];
