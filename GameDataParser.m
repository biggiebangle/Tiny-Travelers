...
+ (GameData *)loadData { 
   ...
   // NSLog(@"Loading %@", filePath);
    ...
    for (GDataXMLElement *element in dataArray) {
        ...
        NSArray *weatherCodeArray = [element elementsForName:@"WeatherCode"];
        NSArray *windSpeedArray = [element elementsForName:@"WindSpeed"];
        NSArray *cloudCoverArray = [element elementsForName:@"CloudCover"];
        NSArray *temperatureArray = [element elementsForName:@"Temperature"];
        ...
        if (weatherCodeArray.count > 0) {
            GDataXMLElement *weatherCodeElement = (GDataXMLElement *) [weatherCodeArray objectAtIndex:0];
            weatherNumber = [[weatherCodeElement stringValue] intValue];
        }
        if (windSpeedArray.count > 0) {
            GDataXMLElement *windSpeedElement = (GDataXMLElement *) [windSpeedArray objectAtIndex:0];
            windSpeed = [[windSpeedElement stringValue] intValue];
        }
        if (cloudCoverArray.count > 0) {
            GDataXMLElement *cloudCoverElement = (GDataXMLElement *) [cloudCoverArray objectAtIndex:0];
            cloudCover = [[cloudCoverElement stringValue] intValue];
        }
        if (temperatureArray.count > 0) {
            GDataXMLElement *temperatureElement = (GDataXMLElement *) [temperatureArray objectAtIndex:0];
            temperature = [[temperatureElement stringValue] intValue];
        }
...
+ (void)saveData:(GameData *)saveData { 
    ...
    GDataXMLElement *weatherCodeElement =
    [GDataXMLNode elementWithName:@"WeatherCode"
                      stringValue:[[NSNumber numberWithInt:saveData.weatherNumber] stringValue]];
    
    GDataXMLElement *windSpeedElement =
    [GDataXMLNode elementWithName:@"WindSpeed"
                      stringValue:[[NSNumber numberWithInt:saveData.windSpeed] stringValue]];
    
    GDataXMLElement *cloudCoverElement =
    [GDataXMLNode elementWithName:@"CloudCover"
                      stringValue:[[NSNumber numberWithInt:saveData.cloudCover] stringValue]];
    
    GDataXMLElement *temperatureElement =
    [GDataXMLNode elementWithName:@"Temperature"
                      stringValue:[[NSNumber numberWithInt:saveData.temperature] stringValue]];
    ...
    [gameDataElement addChild:weatherCodeElement];
    [gameDataElement addChild:windSpeedElement];
    [gameDataElement addChild:cloudCoverElement];
    [gameDataElement addChild:temperatureElement];
    ...
    GDataXMLDocument *document = [[GDataXMLDocument alloc] 
                                   initWithRootElement:gameDataElement];
